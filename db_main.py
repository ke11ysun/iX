...
def sql_table(con):
	...
	cursorObj.execute("CREATE TABLE movies(id integer PRIMARY KEY, name text, genre text, showing integer, year integer, IMDB real, description text, time_ text)")
...
def to_database(con, movie_d):
	cursorObj = con.cursor()
	cursorObj.executemany("INSERT INTO movies (id, name, genre, showing, year, IMDB, description, time_) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", movie_d)
